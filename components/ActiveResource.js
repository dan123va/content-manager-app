...
export default function ActiveResource() {
  ...
  useEffect(() => {
    ...
    if (seconds < 0 || seconds === NaN || seconds === undefined) {
  ...
  const hasResource = resource && resource.title;
    ...
    <div className="active-resource">
      ...
      {hasResource ? (
        <Link href={`/resources/${resource.id}`}>
          <a className="button">Go tu resource</a>
        </Link>
      ) : (
        <Link href="/">
          <a className="button">Go tu resources</a>
        </Link>
      )}
    </div>
  );
